...
class ObstacleDetection():
    ...
    def __init__(self):
        ...
        self.vis_pubFront = rospy.Publisher('/TractorLineFront_pt', Marker, queue_size=2)
        self.vis_pubDown = rospy.Publisher('/TractorLineDown_pt', Marker, queue_size=2)
    ...
    def convertToXDistAndYDistFront(self):
        ...
        for i in range(len(self.frontData.ranges)): # Puts the tuple of data into x and y Distances
            ...
            self.xDistFront.append(abs(math.cos(angleOfIncline) * math.cos(angleRad) * self.totalDistFront[i])) # Computes the distance from the object
            self.yDistFront.append(math.sin(angleRad) * self.totalDistFront[i]) # Computes the distance parallel to tractor
            self.zDistFront.append(math.sin(angleOfIncline) * math.cos(angleRad) * self.totalDistFront[i])
    ...
    def getNumberOfObstaclesFront(self,visualize):
        ...
        for i in range(len(self.totalDistFront)): # Sweep through the distances
            if(self.zDistFront[i] < self.DownToGround): # Is there an object that is not the ground?
        ...
        if visualize:
            ...
            used_pts.type = 8
            used_pts.scale = Vector3(0.02,0.02,0.02)
            used_pts.color = ColorRGBA(0,0,1,1)
            for i in range(len(self.totalDistFront)):
                used_pts.points.append(Point(self.xDistFront[i],self.yDistFront[i],-self.zDistFront[i]))
            self.vis_pubFront.publish(used_pts)
    ...
    def convertToYDistAndZDistDown(self):
        ...
        for i in range(len(self.downData.ranges)): # Puts the tuple of data into x and y Distances
            ...
            angleRad = math.radians((i - dataPoints / 2) * (angleSweep / dataPoints))
            self.zDistDown.append(math.cos(angleRad) * self.totalDistDown[i]) # Computes the distance from the object
            self.yDistDown.append(math.sin(angleRad) * self.totalDistDown[i]) # Computes the distance parallel to tractor
    ...
    def getNumberOfObstaclesDown(self,visualize):
        ...
        if visualize:
            ...
            # self.vis_pubDown.publish(used_pts)
            used_pts = Marker() # Marker to visualize used lidar pts
            used_pts.header.frame_id = "laser"
            used_pts.header.stamp = rospy.Time.now()
            used_pts.type = 8
            used_pts.scale = Vector3(0.02,0.02,0.02)
            used_pts.color = ColorRGBA(1,0,0,1)
            for i in range(len(self.totalDistDown)):
                used_pts.points.append(Point(0,self.yDistDown[i],-self.zDistDown[i]))
            self.vis_pubDown.publish(used_pts)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown() and self.frontData == None and self.downData != None:
            ...
            self.getNumberOfObstaclesDown(True)
        ...
        while not rospy.is_shutdown() and self.downData == None and self.frontData != None:
            ...
            self.getNumberOfObstaclesFront(True)
