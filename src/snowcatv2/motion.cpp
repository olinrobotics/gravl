...
Motion::Motion(ros::NodeHandle *nh, const HardwareSerial *rc, const long baud, const uint8_t address):address(address){
...
void Motion::vel_callback(const geometry_msgs::Twist &message){
	roboclaw.DutyAccelM1(address, s, a);
	roboclaw.DutyAccelM2(address, s, a);
}